...
func TestSession_TelemetryRecordsPongsAndData(t *testing.T) {
	...
	srv, wsURL := newTestServer(t, func(conn *websocket.Conn) {
		...
		go func() {
			...
			conn.WriteMessage(websocket.TextMessage, []byte("tick 2\r\n"))
			close(dataSent)
		}()

		// Keep reading so gorilla auto-pongs the client's pings.
		for {
			if _, _, err := conn.ReadMessage(); err != nil {
				return
			}
	...
	s := New()
	s.pingPeriod = 30 * time.Millisecond
	defer s.Close()
	...
	// Wait for the server to have sent its data lines.
	select {
	case <-dataSent:
	case <-time.After(2 * time.Second):
		t.Fatal("timeout waiting for server to send data")
	}

	// Drain lines and poll stats until both data frames and at least one
	// ping/pong round-trip have been recorded, or time out.
	deadline := time.After(2 * time.Second)
	for {
		select {
		case _, ok := <-s.Lines():
			if !ok {
				t.Fatal("lines channel closed prematurely")
			}
		case <-time.After(10 * time.Millisecond):
			s.stats.mu.Lock()
			pongs := s.stats.pongsRecv
			data := s.stats.dataFrames
			s.stats.mu.Unlock()
			if pongs >= 1 && data >= 1 {
				return
			}
		case <-deadline:
			...
			data := s.stats.dataFrames
			s.stats.mu.Unlock()
			t.Fatalf("timeout waiting for telemetry: pongsRecv=%d dataFrames=%d", pongs, data)
		}
	}
}
